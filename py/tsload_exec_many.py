...
def getData(records_in_batch, sqlConn):
    data = []

    for x in range(records_in_batch):
        restaurant_id = getRestaurantID(sqlConn)
        cpuData = getSampleCPUData()
        ts_insert = datetime.fromtimestamp(time.time()).strftime('%Y-%m-%d %H:%M:%S')
        #tuple
        new_row = (ts_insert,
                   restaurant_id,
...
def insertCPUValues():
    ...
    for x in range(1):
        cursor = sqlConn.cursor()
        data = getData(11, sqlConn)
        #test table: sql_test
        query = ("INSERT INTO cpu_test "
                 "(ts_insert, restaurant_id,virtual_memory_percent,virtual_memory_avail,cpu_percent,getloadavg) "
                 "VALUES (%s, %s, %s, %s, %s, %s);")
